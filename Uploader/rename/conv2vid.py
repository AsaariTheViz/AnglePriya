...
async def convert_to_video(bot, update):
    ...
    if update.reply_to_message:
        ...
        if file_path:
            ...
            width, height, duration = await Get_video_dimensions(file_path)
            if thumb_id:
                ...
                # ph_path = None
                thumb = await Take_screen_shot(file_path, thumb_path)
            ...
            thumb_id = find_any(USER, "PHOTO_THUMB")
            if not thumb_id:
                ph_path = thumb = await Take_screen_shot(file_path, thumb_path)
